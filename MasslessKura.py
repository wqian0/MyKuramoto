...
N, alpha, dt, frequencyBound, steps, startDelay, endDelay= gg.size, .15, .02, gg.freqBound, 50000, gg.numTransitionGraphs // 3, gg.numTransitionGraphs // 3
m = 0
oParameterData=[]
standardOPData =[]
localOPData = []
...
def get_AList(start=None, end=None, final=None, dens_const=True):
    if start is not None:
        gg.main(start=start, end=end, final=final, dens_const=dens_const)
    else:
        gg.main()
...
# plt.pause(15)
# for i in range(0, len(timeArray), 20):
#     plt.clf()
#     plt.xlabel('phase')
...
TA_OP_file = open("massless_TA_OP_const_dens "+str(arg_1)+".txt", "w")
